...
interface Props {
	children: Array<
		React.ReactElement<typeof TilesInvisible | typeof TilesVisible>
	>
	img?: string
}

const LazyCodeScrenImage = lazy(async () =>
	import('../lazyImages/LazyCodeScreen').then(m => ({
		default: m.LazyCodeScrenImage
	}))
)
const LazyInventroyBoxes = lazy(async () =>
	import('../lazyImages/LazyInventoryBoxes').then(m => ({
		default: m.LazyInventroyBoxes
	}))
)
const LazyOfficeNotebookImage = lazy(async () =>
	import('../lazyImages/LazyOfficeNoteBookImg').then(m => ({
		default: m.LazyOfficeNotebookImage
	}))
)
const LazyOfficeTableDeskImg = lazy(async () =>
	import('../lazyImages/LazyOfficeTableDeskImg').then(m => ({
		default: m.LazyOfficeTableDeskImg
	}))
)
const LazyDefaultImg = lazy(async () =>
	import('../lazyImages/LazyDefaultImg').then(m => ({
		default: m.LazyDefaultImg
	}))
)
const LazyOfficeImg = lazy(async () =>
	import('../lazyImages/LazyOffice1Image').then(m => ({
		default: m.LazyOfficeImage
	}))
)
...
export function TilesBox({ children, img }: Props & { img: TypeOFImg }) {
	...
	return (
		<div className="overflow-hidden odd:text-white even:text-black group hover:before:-top-3/4 before:will-change-transform before:origin-center before:-rotate-[25deg] before:transition-all before:duration-700 before:ease-in-out before:absolute before:top-[200%] before:-left-3/4 before:w-[250%] before:block before:h-[220%] before:bg-primary/85 relative w-full min-h72 h-72">
			<Suspense
				fallback={
					<div className="w-full h-full animate-pulse bg-slate-400" />
				}
			>
