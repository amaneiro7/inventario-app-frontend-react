...
const LocationInputs = lazy(async () =>
	import('../../page/FormLocation/LocationInputs').then(m => ({
...
export function LocationDialog({ handleClose, initialDialogValue }: Props) {
	const {
		...
		error,
		required
	} = useFormLocation(initialDialogValue)
			...
			>
				<Suspense>
					<Subtitle
						...
						text="Agregar una nueva ubicación"
					/>
					<Paragraph
						variant="p"
						color="gray"
						text="¿No existe la ubicación en la lista? Por favor, añada uno nuevo."
					/>
				</Suspense>
				<Suspense>
					<LocationInputs
