...
const DialogComponent = lazy(async () =>
	import('@/sections/components/Dialog/DialogComponent').then(m => ({
		default: m.DialogWrapper
	}))
)
const ComboBox = lazy(async () => import('./combo_box'))
const LocationDialog = lazy(async () =>
	import('../Dialog/LocationDialog').then(m => ({
		default: m.LocationDialog
	}))
)
...
export default function LocationComboBox({
	...
	const {
		useSiteLocation: { locations, loading }
	} = useAppContext()
	const [dialogValue, setDialogValue] =
		useState<DefaultLocationProps>(initialLocationState)
	const [open, setOpen] = useState(false)

	const filterLocation = useMemo(() => {
		return locations.filter(location => {
			const typeOfSite =
				location.typeOfSiteId === typeOfSiteId || !typeOfSiteId
			const status = !statusId
				...
				: [
					...
					? [
							TypeOfSiteId.SitesOptions.ADMINISTRATIVE,
							TypeOfSiteId.SitesOptions.AGENCY
						].includes(location.typeOfSiteId)
					: [TypeOfSiteId.SitesOptions.ALMACEN].includes(
							location.typeOfSiteId
						)
