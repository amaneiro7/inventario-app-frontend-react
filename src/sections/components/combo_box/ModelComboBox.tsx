...
const ReadOnlyInputBox = lazy(async () =>
	import('../ReadOnlyInputBox').then(m => ({ default: m.ReadOnlyInputBox }))
)
...
export default function ModelComboBox({
	...
	const {
		useModel: { models, loading }
	} = useAppContext()
	const [open, toggleOpen] = useState(false)
	const [dialogValue, setDialogValue] =
		useState<DefaultModelProps>(initialModelState)
							...
							if (typeof newValue === 'string') {
								...
								if (type === 'form') {
									...
									if (newValue?.modelComputer !== null) {
										memoryRamSlotQuantity = newValue
											? newValue?.modelComputer
													.memoryRamSlotQuantity
											: undefined
										memoryRamType = newValue
											? newValue?.modelComputer
													.memoryRamType.name
											: ''
										handleModel({
