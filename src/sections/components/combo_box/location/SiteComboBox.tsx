...
const DialogWrapper = lazy(async () =>
...
const SiteDialog = lazy(async () =>
	import('@/sections/components/Dialog/SiteDialog').then(m => ({
		default: m.SiteDialog
	}))
)
...
export function SiteComboBox({
	...
	const {
		useSite: { sites, loading }
	} = useAppContext()
	const [dialogValue, setDialogValue] =
		useState<DefaultSiteProps>(initialSiteState)
