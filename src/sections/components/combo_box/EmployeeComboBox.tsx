...
const EmployeeDialog = lazy(async () =>
	import('../Dialog/EmployeeDialog').then(m => ({
		default: m.EmployeeDialog
	}))
)
...
export default function EmployeeComboBox({
	...
	const {
		useEmployee: { employees, loading }
	} = useAppContext()
	const [dialogValue, setDialogValue] =
		useState<DefaultEmployeeProps>(initialEmployeeState)
