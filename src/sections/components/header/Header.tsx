...
const ConfirmationModal = lazy(async () =>
	import('../Dialog/ConfirmationModal').then(m => ({
...
const Modal = lazy(async () =>
	import('../Dialog/Modal').then(m => ({ default: m.Modal }))
)
...
export const Header = memo(function () {
	...
	const {
		useAuth: { logout, user: userDefault }
	} = useAuthContext()
			...
			<header className="min-h-16 h-16 md:text-sm md:border-none gap-4 flex items-center justify-between md:top-0 md:sticky z-50 bg-secondary w-full shadow-lg pr-8 py-4 overflow-visible">
				...
				<div className="flex flex-1 gap-8 items-center justify-end">
					{User.isSuperAdmin({ roleId: user?.roleId }) && (
						<Link
							to="/user-management"
							className="text-white text-xs md:text-sm lg:text-base font-medium p-1 border-b hover:text-primary hover:border-primary transition-colors duration-200"
						>
							Gestión de Usuarios
						</Link>
					)}
