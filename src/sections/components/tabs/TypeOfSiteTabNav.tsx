...
const TabsNav = lazy(async () =>
	import('./TabsNav').then(m => ({ default: m.TabsNav }))
)
const TabNav = lazy(async () =>
	import('./TabNav').then(m => ({ default: m.TabNav }))
)

export const TypeOfSiteTabNav = memo(function ({ onChange, value }: Props) {
	const [inputValue, setInputValue] = useState(() => (value ? value : '0'))
	const {
		useTypeOfSite: { typeOfSite, loading }
	} = useAppContext()
