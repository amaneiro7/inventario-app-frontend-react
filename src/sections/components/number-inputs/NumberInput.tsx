	...
	return (
		<div className="relative inline-flex flex-col align-top w-full select-all">
			...
			>
				...
				<fieldset
					aria-hidden
					className={`text-left absolute inset-0 -top-1 m-0 p-0 pointer-events-none overflow-hidden min-w-0
         `}
				>
					<legend
						style={{
					...
					>
						<span className="visible px-1 inline-block opacity-0">
							{`${label} ${isRequired ? '*' : ''}`}
						</span>
					</legend>
				</fieldset>
			...
			{error && (
				<p className="text-xs m-1 mx-4 font-normal font-sans text-left tracking-[0.033rem] text-error">
					{errorMessage}
				</p>
			)}
		</div>
