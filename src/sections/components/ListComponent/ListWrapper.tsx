...
const ButtonSection = lazy(async () =>
	import('./buttonsection/ButtonSection').then(m => ({
		default: m.ButtonSection
	}))
)
const TypeOfSiteTabNav = lazy(async () =>
	import('../tabs/TypeOfSiteTabNav').then(m => ({
		default: m.TypeOfSiteTabNav
	}))
)
const PaginationBar = lazy(async () =>
	import('../Pagination/PaginationBar').then(m => ({
		default: m.PaginationBar
	}))
)
...
export function ListWrapper({
	...
	const { download, isDownloading } = useDownloadExcelFromServer({
		query,
		source
	})

	const handleFilter = () => {
		filterContainerRef.current?.handleOpen()
	}
