...
export const DeviceContextProvider = ({
	children,
	location
}: React.PropsWithChildren<{ location?: LocationProps }>) => {
	...
	const { devices, total, error, loading, searchDevices, resetDevices } =
		useSearchDevice()
	...
	// the search form.
	const { addFilter, cleanFilters, query } =
		useSearchByCriteriaQuery(defaultCategoryQuery)
