...
export function useErrorManagement({
	...
	useEffect(() => {
		setError(prev => ({
			...prev,
			serial: DeviceSerial.isValid(serial)
				? ''
				: DeviceSerial.invalidMessage(),
			activo: DeviceActivo.isValid(activo)
				? ''
			...
			employeeId: DeviceEmployee.isValid(employeeId, statusId)
				? ''
				: DeviceEmployee.invalidMessage(),
			stockNumber: DeviceStockNumber.isValid(stockNumber, statusId)
				? ''
				: DeviceStockNumber.invalidMessage(),
			computerName: ComputerName.isValid(computerName, statusId)
				? ''
				: ComputerName.invalidMessage(),
			ipAddress: IPAddress.isValid(ipAddress, statusId)
				? ''
				: IPAddress.invalidMessage(),
			memoryRamCapacity: MemoryRam.isValid(memoryRam, statusId)
				? ''
				: MemoryRam.invalidMessage(),
			processorId: ComputerProcessor.isValid(processorId, statusId)
				? ''
			...
			macAddress: MACAddress.isValid(macAddress)
				? ''
				: MACAddress.invalidMessage(macAddress),
			health: HardDriveHealth.isValid(health)
				? ''
		...
		setDisabled(prev => ({
			...
			locationId:
				!statusId ||
				[StatusId.StatusOptions.DESINCORPORADO].includes(statusId),
			stockNumber:
				!statusId ||
				![
			...
			operatingSystemId:
				[
					...
					StatusId.StatusOptions.PORDESINCORPORAR,
					StatusId.StatusOptions.DESINCORPORADO
				].includes(statusId) || !hardDriveCapacityId,
		...
		setRequired(prev => ({
			...
			locationId: ![StatusId.StatusOptions.DESINCORPORADO].includes(
				statusId
			),
			computerName: ![
				StatusId.StatusOptions.INALMACEN,
