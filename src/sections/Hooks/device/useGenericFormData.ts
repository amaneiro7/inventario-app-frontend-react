...
const reducer = (state: DefaultProps, action: Action): DefaultProps => {
	...
	if (action.type === 'modelId') {
		const { value, memoryRamSlotQuantity, memoryRamType, generic } =
			action.payload

		const memoryRam =
			state.memoryRam?.length === memoryRamSlotQuantity
				? state.memoryRam
...
export function useFormDevice() {
	...
	const { isAddForm, preloadedDeviceState, setResetState } =
		useDeviceInitialState()
	...
	useLayoutEffect(() => {
		dispatch({
			type: 'INIT_STATE',
			payload: { formData: structuredClone(preloadedDeviceState) }
		})
