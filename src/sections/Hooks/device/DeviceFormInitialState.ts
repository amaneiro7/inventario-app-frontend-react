...
export const useDeviceInitialState = (): {
	...
	const [preloadedDeviceState, setPreloadedDeviceState] =
		useState(defaultInitialState)

	const isAddForm = useMemo(() => {
		return !location.pathname.includes('edit')
	}, [location.pathname])

	const processDeviceState = useCallback(
		(device: DevicePrimitives): void => {
			const {
				serial,
				activo,
				statusId,
			...
			} = device as DevicesApiResponse
			setPreloadedDeviceState(prev => ({
				...prev,
				id,
				serial,
				activo: activo ?? '',
			...
			if (computer !== null) {
				...
				const meRam =
					memoryRam.length !== memoryRamSlotQuantity
						? [
								...memoryRam,
								...Array(
									memoryRamSlotQuantity - memoryRam.length
								).fill(0)
							]
						: memoryRam
			...
			if (hardDrive !== null) {
				const { health, hardDriveCapacityId, hardDriveTypeId } =
					hardDrive
				setPreloadedDeviceState(prev => ({
					...prev,
					health,
