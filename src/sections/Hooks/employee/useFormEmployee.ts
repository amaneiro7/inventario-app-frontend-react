...
export function useFormEmployee(defaultInitialState?: DefaultEmployeeProps) {
	const {
		useEmployee: { createEmployee }
	} = useAppContext()
	const { preloadedEmployeeState, isAddForm, setResetState } =
		useEmployeeInitialState(defaultInitialState ?? initialEmployeeState)
	...
	useLayoutEffect(() => {
		dispatch({
			type: 'init',
			payload: { formData: structuredClone(preloadedEmployeeState) }
		})
