...
import {
	type FormEmployeeDisabled,
	type FormEmployeeErrors,
	FormEmployeeRequired,
	type DefaultEmployeeProps
} from '@/sections/Hooks/employee/DefaultInitialState'
...
export function useErrorEmployeeManagement({ userName }: DefaultEmployeeProps) {
	...
	useEffect(() => {
		if (isFirstUserNameInput.current || userName === '') {
			isFirstUserNameInput.current =
				userName.length < EmployeeUserName.NAME_MIN_LENGTH
		}
		setError(prev => ({
			...prev,
			userName: isFirstUserNameInput.current
				? ''
				: EmployeeUserName.isValid(userName)
					? ''
					: EmployeeUserName.invalidMessage(userName)
		}))
		setDisabled(prev => ({
			...prev
		}))
