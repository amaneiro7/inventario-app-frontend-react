...
export const useEmployeeByCriteria = (
	defaultQuery?: SearchByCriteriaQuery
): UseEmployee => {
	const { query, addFilter, cleanFilters } =
		useSearchByCriteriaQuery(defaultQuery)
	const [loading, setLoading] = useState<boolean>(true)
	const [error, setError] = useState<string | null>(null)
	const [employeeWithDevives, setEmployeeWithDevives] = useState<
		EmployeePrimitives[]
	>([])
