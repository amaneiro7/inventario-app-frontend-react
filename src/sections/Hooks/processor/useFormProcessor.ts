...
export function useFormProcessor(defaultInitialState?: DefaultProcessorProps) {
	const {
		useProcessor: { createProcessor }
	} = useAppContext()
	const { preloadedProcessorState, isAddForm, setResetState } =
		useProcessorInitialState(defaultInitialState ?? initialProcessorState)
	...
	const { processing, submitForm } = useGenericForm2({
	...
	useLayoutEffect(() => {
		dispatch({
			type: 'init',
			payload: { formData: structuredClone(preloadedProcessorState) }
		})
	...
	const resetForm = () => {
		dispatch({
			type: 'reset',
			payload: { formData: structuredClone(prevFormData) }
		})
	}
