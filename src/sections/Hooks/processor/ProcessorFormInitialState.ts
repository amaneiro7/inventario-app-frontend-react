...
export const useProcessorInitialState = (
	defaultInitialProcessorState: DefaultProcessorProps
) => {
	const { id } = useParams()
	const location = useLocation()
	const navigate = useNavigate()
	const { getProcessor } = useProcessor()
	const [preloadedProcessorState, setPreloadedProcessorState] = useState(
		defaultInitialProcessorState
	)
