...
export function useErrorProcessorManagement({
	...
	useEffect(() => {
		if (isFirstProcessorNameInput.current || name === '') {
			isFirstProcessorNameInput.current =
				name.length < ProcessorName.NAME_MIN_LENGTH
		}
		...
		setError(prev => ({
			...prev,
			name: isFirstProcessorNameInput.current
				? ''
				: ProcessorName.isValid(name)
					? ''
					: ProcessorName.invalidMessage(name),
			productCollection: isFirstProductCollectionInput.current
				? ''
				: ProcessorProductCollection.isValid(productCollection)
					? ''
					: ProcessorProductCollection.invalidMessage(
							productCollection
						),
			numberModel: isFirstNumberModelInput.current
				? ''
				: ProcessorNumberModel.isValid(productCollection)
					? ''
					: ProcessorNumberModel.invalidMessage(productCollection),
			cores: ProcessorCores.isValid(cores)
				? ''
				: ProcessorCores.invalidMessage(cores),
			frequency: ProcessorFrequency.isValid(frequency)
				? ''
				: ProcessorFrequency.invalidMessage(frequency),
			threads: ProcessorHasThreads.isValid(threads)
				? ''
				: ProcessorHasThreads.invalidMessage(threads)
		}))
		setDisabled(prev => ({
			...prev
		}))
