...
export const useModelInitialState = (
	defaultInitialModelState: DefaultModelProps
) => {
	const { id } = useParams()
	const location = useLocation()
	const navigate = useNavigate()
	const { getModel } = useModel()
	const [preloadedModelState, setPreloadedModelState] = useState(
		defaultInitialModelState
	)
	...
	const processModelState = useCallback(
		(model: ModelPrimitives): void => {
			const {
				brandId,
				categoryId,
				name,
				...
				modelMonitor,
				modelPrinter
			} = model as ModelApiresponse
			setPreloadedModelState(prev => ({
				...prev,
				id,
				brandId,
				categoryId,
