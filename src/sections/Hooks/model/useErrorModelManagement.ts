...
export function useErrorModelManagement({
	...
	useEffect(() => {
		...
		if (isFirstBatteryModelInput.current || batteryModel === '') {
			isFirstBatteryModelInput.current =
				batteryModel?.length < BatteryModel.NAME_MIN_LENGTH
		}
		if (isFirstCartridgeModel.current || cartridgeModel === '') {
			isFirstCartridgeModel.current =
				cartridgeModel?.length < CartridgeModel.NAME_MIN_LENGTH
		}
		setError(prev => ({
			...prev,
			name: isFistNameInput.current
				? ''
				: ModelName.isValid(name)
					? ''
					: ModelName.invalidMessage(name),
			batteryModel: isFirstBatteryModelInput.current
				? ''
				: BatteryModel.isValid(batteryModel)
					? ''
					: BatteryModel.invalidMessage(batteryModel),
			cartridgeModel: isFirstCartridgeModel.current
				? ''
				: CartridgeModel.isValid(cartridgeModel)
					? ''
					: CartridgeModel.invalidMessage(cartridgeModel),
			memoryRamSlotQuantity: MemoryRamSlotQuantity.isValid(
				memoryRamSlotQuantity
			)
				? ''
				: MemoryRamSlotQuantity.invalidMessage(),
			screenSize: ScreenSize.isValid(screenSize)
				? ''
				: ScreenSize.invalidMessage()
		}))
		setDisabled(prev => ({
			...prev,
			brandId: !categoryId
