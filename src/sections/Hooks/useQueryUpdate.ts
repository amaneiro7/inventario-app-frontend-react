...
export const useSearchByCriteriaQuery = (
	defaultQuery?: SearchByCriteriaQuery
) => {
		...
		(payload: SearchByCriteriaQuery) => {
			setQuery(prevQuery => {
				...
				if (payload?.filters?.length > 0) {
					...
					const isDefaultFilter = defaultQuery
						? defaultQuery?.filters?.some(
								df => df.field === filterToAdd.field
							)
						: false
					// Find the index of the existing filter with the same field, if any
					const filterIndex = prevQuery.filters.findIndex(
						filter => filter.field === filterToAdd.field
					)
					...
					if (!isDefaultFilter) {
						...
						newQuery.filters = newQuery.filters.filter(
							filter => filter.field !== filterToAdd.field
						)
						...
						if (filterToAdd.value !== '') {
							...
							defaultQuery?.filters.forEach(filter => {
				...
				const filterWithoutEmptyValues = newQuery.filters.filter(
					filter => filter.value !== ''
				)
				...
				return {
					...prevQuery,
					...newQuery,
					filters: filterWithoutEmptyValues
				}
			})
		},
		[defaultQuery]
