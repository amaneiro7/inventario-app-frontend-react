...
export const useLocationInitialState = (
	defaultInitialLocationState: DefaultLocationProps
) => {
	const { id } = useParams()
	const location = useLocation()
	const navidate = useNavigate()
	const { getLocation } = useSiteLocation()
	const [preloadedLocationState, setPreloadedLocationState] = useState(
		defaultInitialLocationState
	)
	...
	const processLocationState = useCallback(
		(location: LocationPrimitives): void => {
			const { id, name, site, siteId, subnet, typeOfSiteId } =
				location as LocationApiResponse
			const { name: siteName, cityId, city } = site
			const { state, stateId } = city
			const { regionId } = state
			setPreloadedLocationState(prev => ({
				...prev,
				id,
				name,
				siteId,
