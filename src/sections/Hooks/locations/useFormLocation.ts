...
export function useFormLocation(defaultInitialState?: DefaultLocationProps) {
	const {
		useSiteLocation: { createLocation }
	} = useAppContext()
	const { isAddForm, preloadedLocationState, setResetState } =
		useLocationInitialState(defaultInitialState ?? initialLocationState)
	...
	useLayoutEffect(() => {
		dispatch({
			type: 'init',
			payload: { formData: structuredClone(preloadedLocationState) }
		})
