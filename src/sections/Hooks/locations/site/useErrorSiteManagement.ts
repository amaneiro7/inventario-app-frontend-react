...
import {
	type DefaultSiteProps,
	type FormSiteDisabled,
	type FormSiteErrors,
	type FormSiteRequired
} from '@/sections/Hooks/locations/site/DefaultSiteInitialState'
...
export function useErrorSiteManagement({ name, address }: DefaultSiteProps) {
	...
	useEffect(() => {
		if (isFirstSiteNameInput.current || name === '') {
			isFirstSiteNameInput.current =
				name.length < SiteName.NAME_MIN_LENGTH
		}
		if (isFirstSiteAddressInput.current || address === '') {
			isFirstSiteAddressInput.current =
				name.length < SiteAddress.NAME_MIN_LENGTH
		}
		setError(prev => ({
			...prev,
			name: isFirstSiteNameInput.current
				? ''
				: SiteName.isValid(name)
					? ''
					: SiteName.invalidMessage(),
			address: isFirstSiteAddressInput.current
				? ''
				: SiteAddress.isValid(name)
					? ''
					: SiteAddress.invalidMessage()
		}))
		setDisabled(prev => ({
			...prev
		}))
