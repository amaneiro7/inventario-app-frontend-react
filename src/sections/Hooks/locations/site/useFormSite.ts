...
export function useFormSite(defaultInitialState?: DefaultSiteProps) {
	...
	const { preloadedSiteState, isAddForm, setResetState } =
		useSiteInitialState(defaultInitialState ?? initialSiteState)
	...
	useLayoutEffect(() => {
		dispatch({
			type: 'init',
			payload: { formData: structuredClone(preloadedSiteState) }
		})
