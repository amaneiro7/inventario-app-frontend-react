	...
	useEffect(() => {
		if (isFirstSiteNameInput.current || siteName === '') {
			isFirstSiteNameInput.current =
				siteName.length < LocationName.NAME_MIN_LENGTH
		}
		...
		setError(prev => ({
			...prev,
			siteName: isFirstSiteNameInput.current
				? ''
				: LocationName.isValid(siteName)
					? ''
					: LocationName.invalidMessage(),
			subnet: isFirstSubnetInput.current
				? ''
				: Subnet.isValid(subnet)
					? ''
					: Subnet.invalidMessage(),
			codeAgency:
