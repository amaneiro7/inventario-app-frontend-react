...
export function clearModelDataset({
	models
}: {
	...
	return models.map(model => ({
		...
		'Tamaño de Pantalla': model?.modelMonitor
			? `${model?.modelMonitor.screenSize} pulgadas`
			: 'N/A',
		'Modelo de Cartucho': model?.modelPrinter?.cartridgeModel ?? 'N/A',
		'Tipo de entrada': model?.modelKeyboard?.inputType?.name ?? 'N/A',
		'Lector de Huella': model?.modelKeyboard
			? model?.modelKeyboard?.hasFingerPrintReader
				? 'Si'
				: 'No'
			: 'N/A',
