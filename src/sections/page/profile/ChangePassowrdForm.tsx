...
const CancelIcon = lazy(() =>
	import('@/sections/components/icon/CancelIcon').then(m => ({
		default: m.CancelIcon
	}))
)
const RightArrowIcon = lazy(() =>
	import('@/sections/components/icon/RightArrowIcon').then(m => ({
		default: m.RightArrowIcon
	}))
)
const LockIcon = lazy(
	async () =>
		await import('../../components/icon/LockIcon').then(m => ({
			default: m.LockIcon
		}))
)
const UnlockIcon = lazy(
	async () =>
		await import('../../components/icon/UnlockIcon').then(m => ({
...
export function ChangePassowrdForm({
	...
	const handleToogleInputs = (
		name: 'password' | 'newPassword' | 'reTypePassword'
	) => {
		setToogleInputs(prevState => ({
			...prevState,
			[name]: !prevState[name]
		}))
	}
		...
		>
			<div className="flex flex-col gap-4">
				...
				<Input
					...
					onRightIconClick={() =>
						handleToogleInputs('reTypePassword')
					}
				/>
			</div>
