	...
	useEffect(() => {
		...
		if (isNewPasswordFirstInput.current || formData.newPassword === '') {
			isNewPasswordFirstInput.current =
				formData.newPassword?.length <= UserPassword.HAS_MIN_LENGTH
		}
		if (
			...
			isReTypePasswordFirstInput.current =
				formData.reTypePassword?.length <= UserPassword.HAS_MIN_LENGTH
		}
		...
		const isNewPasswordValid = isNewPasswordFirstInput.current
			? true
			: UserPassword.isValid(formData.newPassword)
