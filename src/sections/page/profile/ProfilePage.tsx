...
const ChangePasswordStepsToFollow = lazy(async () =>
	import('./ChangePasswordStepsToFollow').then(m => ({
		default: m.ChangePasswordStepsToFollow
	}))
)
const ConfirmationModal = lazy(async () =>
	import('@/sections/components/Dialog/ConfirmationModal').then(m => ({
...
const DescriptionListElement = lazy(async () =>
	import('@/sections/components/DetailsWrapper/DescriptionListElement').then(
		m => ({ default: m.DescriptionListElement })
	)
)
...
const Modal = lazy(async () =>
	import('@/sections/components/Dialog/Modal').then(m => ({
		default: m.Modal
	}))
)
const ChangePassowrdForm = lazy(async () =>
	import('./ChangePassowrdForm').then(m => ({
...
export default function ProfilePage() {
	...
	const {
		user: { name, email, lastName, role }
	} = user as unknown as UserApiResponse
			...
			<DetailsWrapper title="A continuación le indicamos los datos de contacto">
				<DetailsInfo title="Datos de Contacto">
					<DescriptionListElement title="Nombre">
						<DescriptionDesc desc={name} />
					</DescriptionListElement>
					<DescriptionListElement title="Apellido">
						<DescriptionDesc desc={lastName} />
					</DescriptionListElement>
					<DescriptionListElement title="Correo">
						<DescriptionDesc desc={email} />
					</DescriptionListElement>
					<DescriptionListElement title="Role">
						<DescriptionDesc desc={role?.name} />
					</DescriptionListElement>
				</DetailsInfo>
