...
export function useRegisterPage(): {
	...
	const { formData, updateForm, resetForm } =
		useGenericFormData(preloadedState)
	const { formStatus, resetFormStatus, submitForm } = useGenericForm({
		create: createUser
	})
	...
	useEffect(() => {
		...
		if (isNameFirstInput.current || formData.name === '') {
			isNameFirstInput.current =
				formData.name?.length <= UserName.NAME_MIN_LENGTH
		}
		if (isLastNameFirstInput.current || formData.name === '') {
			isLastNameFirstInput.current =
				formData.lastName?.length <= UserLastName.NAME_MIN_LENGTH
		}

		const isNameValid = isNameFirstInput.current
			? true
			: UserName.isValid(formData.name)
		const isLastNameValid = isLastNameFirstInput.current
			? true
			: UserLastName.isValid(formData.lastName)
