...
import {
	...
	type FormModelDisabled,
	type FormModelErrors,
	type FormModelRequired
} from '@/sections/Hooks/model/DefaultInitialModelState'
...
const Input = lazy(async () =>
	import('@/sections/components/text-inputs/Input').then(m => ({
		default: m.Input
	}))
)
const CategoryComboBox = lazy(
	async () => import('@/sections/components/combo_box/CategoryComboBox')
)
const BrandComboBox = lazy(
	async () => import('@/sections/components/combo_box/BrandComboBox')
)
const AddModelComputer = lazy(async () =>
	import('./AddModelComputer').then(m => ({ default: m.AddModelComputer }))
)
const AddModelMonitor = lazy(async () =>
	import('./AddModelMonitor').then(m => ({ default: m.AddModelMonitor }))
)
const AddModelPrinter = lazy(async () =>
	import('./AddModelPrinter').then(m => ({ default: m.AddModelPrinter }))
)
const AddModelKeyboard = lazy(async () =>
	import('./AddModelKeyboard').then(m => ({ default: m.AddModelKeyboard }))
)
const Checkbox = lazy(async () =>
	import('@/sections/components/checkbox/Checbox').then(m => ({
		default: m.Checkbox
	}))
)
...
export function ModelInputs({
	...
	const categoryType = useMemo(() => {
		return ModelComputer.isComputerCategory({
			categoryId: formData.categoryId
		})
			? 'computer'
			: ModelLaptop.isLaptopCategory({ categoryId: formData.categoryId })
				? 'laptop'
				: ModelMonitor.isMonitorCategory({
							categoryId: formData.categoryId
					  })
					? 'monitor'
					: ModelPrinter.isPrinterCategory({
								categoryId: formData.categoryId
						  })
						? 'printer'
						: ModelKeyboard.isKeyboardCategory({
									categoryId: formData.categoryId
							  })
							? 'keyboard'
							: null
