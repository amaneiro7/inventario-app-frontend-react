...
const DetailsWrapper = lazy(async () =>
	import('@/sections/components/DetailsWrapper/DetailsWrapper').then(m => ({
...
const InfoBox = lazy(async () =>
	import('@/sections/components/info-box/InfoBox').then(m => ({
		default: m.InfoBox
	}))
)
const InfoBoxTitle = lazy(async () =>
	import('@/sections/components/info-box/InfoBoxTitle').then(m => ({
		default: m.InfoBoxTitle
	}))
)
const InfoBoxText = lazy(async () =>
	import('@/sections/components/info-box/InfoBoxText').then(m => ({
		default: m.InfoBoxText
	}))
)
const TypeOfSiteComboBox = lazy(async () =>
	import('@/sections/components/combo_box/TypeOfSiteComboBox').then(m => ({
		default: m.TypeOfSiteComboBox
	}))
)
const LocationNameInput = lazy(async () =>
	import('@/sections/components/text-inputs/location/LocationNameInput').then(
		m => ({ default: m.LocationNameInput })
	)
)
const Button = lazy(async () => import('@/sections/components/button/button'))
const StateComboBox = lazy(async () =>
	import('@/sections/components/combo_box/location/StateComboBox').then(
		m => ({ default: m.StateComboBox })
	)
)
const RegionComboBox = lazy(async () =>
	import('@/sections/components/combo_box/location/RegionComboBox').then(
		m => ({ default: m.RegionComboBox })
	)
)
const CityComboBox = lazy(async () =>
	import('@/sections/components/combo_box/location/CityComboBox').then(m => ({
		default: m.CityComboBox
	}))
)
const HeaderInput = lazy(async () =>
	import('@/sections/components/HeaderInput').then(m => ({
		default: m.HeaderInput
	}))
)
const Main = lazy(async () => import('@/sections/components/Main'))
const PageTitle = lazy(
	async () => import('@/sections/components/Typography/PageTitle')
)
const AddIcon = lazy(() =>
	import('@/sections/components/icon/AddIcon').then(m => ({
		default: m.AddIcon
	}))
)
...
export default function ListadoSitios() {
	...
	const { inputData: initialInputData, defaultInputData } =
		useDefaultInitialInputValue()
	const { locations, loading, addFilter, cleanFilters } = useLocationContext()
	const { handleChange, handleClear, inputData } = useInputsData({
		addFilter,
		cleanFilters,
		initialInputData,
		defaultInputData
			...
			<Main content="max" overflow={false} className="pr-8">
				...
				<DetailsWrapper borderColor="blue">
					...
					>
						...
						{!loading &&
							locations.length > 0 &&
							(locations as LocationApiResponse[]).map(
								location => (
