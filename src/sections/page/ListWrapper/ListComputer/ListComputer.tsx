...
const ListWrapper = lazy(() =>
	import('@/sections/components/ListComponent/ListWrapper').then(m => ({
		default: m.ListWrapper
	}))
)
const MainComputerFilter = lazy(async () =>
	import('@/sections/components/ListComponent/MainComputerFilter').then(
		m => ({ default: m.MainComputerFilter })
	)
)
const DefaultFilterSection = lazy(() =>
	import('@/sections/components/ListComponent/DefaultFilter').then(m => ({
		default: m.DefaultFilterSection
	}))
)
const OtherComputerFilter = lazy(() =>
	import('@/sections/components/ListComponent/OtherComputerFilter').then(
		m => ({ default: m.OtherComputerFilter })
	)
)
const DeviceTable = lazy(() =>
	import('./TableDevice').then(m => ({ default: m.TableWrapper }))
)

export default function ListComputer() {
	const { inputData: initialInputData, defaultInputData } =
		useDefaultInitialInputValue()
	const { devices, loading, addFilter, cleanFilters, query, managePage } =
		useDeviceContext()
	const { inputData, handleChange, handleClear } = useInputsData({
		initialInputData,
		defaultInputData,
		addFilter,
		cleanFilters
			...
			<ListWrapper
					...
					<Suspense>
						...
						<OtherComputerFilter
							...
							operatingSystemArqId={
								inputData.operatingSystemArqId
							}
				...
				table={
					<DeviceTable
						...
						loading={loading}
						devices={devices as DevicesApiResponse[]}
					/>
				}
			/>
		</Suspense>
