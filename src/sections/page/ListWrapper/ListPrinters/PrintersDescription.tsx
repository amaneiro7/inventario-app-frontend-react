...
const TableCell = lazy(async () =>
	import('@/sections/components/Table/TableCell').then(m => ({
		default: m.TableCell
	}))
)
const TableRow = lazy(async () =>
	import('@/sections/components/Table/TableRow').then(m => ({
		default: m.TableRow
	}))
)
const TableCellDescInfo = lazy(async () =>
	import('@/sections/components/Table/TableCellDescInfo').then(m => ({
		default: m.TableCellDescInfo
	}))
)
const TableCellDescription = lazy(async () =>
	import('@/sections/components/Table/TableCellDescription').then(m => ({
		default: m.TableCellDescription
	}))
)
const TableCellOpenIcon = lazy(async () =>
	import('@/sections/components/Table/TableCellOpenIcon').then(m => ({
		default: m.TableCellOpenIcon
	}))
)

export function PrinterDescription({ devices }: Props) {
		...
		<>
			{devices?.map(device => (
				<React.Fragment key={device.id}>
					<TableRow
						className={`[&>td]:cursor-pointer ${expandedRows.includes(device.id) && '[&>td]:bg-slate-200 [&>td]:border-b-slate-200'}`}
					...
					<Suspense>
						...
						>
							...
							/>
							<TableCellDescInfo
								title="Modelo de cartucho"
								text={
									device?.model?.modelPrinter?.cartridgeModel
								}
							/>
