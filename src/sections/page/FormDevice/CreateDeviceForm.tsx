...
const DeviceSearchComboBox = lazy(
	async () =>
		import('@/sections/components/combo_box/search/DeviceSearchComboBox')
)
const MainFormInputs = lazy(
	async () =>
		await import('./MainFormInputs').then(m => ({
			default: m.MainFormInputs
		}))
)
const AddComputerFeatures = lazy(
	async () => await import('./AddComputerFeatures')
)
const AddHardDriveFeatures = lazy(
	async () => await import('./AddHardDriveFeatures')
)
...
export default function CreateDeviceForm() {
	...
	const categoryType = useMemo(() => {
		return Computer.isComputerCategory({ categoryId: formData.categoryId })
			? 'computer'
			: HardDrive.isHardDriveCategory({ categoryId: formData.categoryId })
				? 'hardDrive'
				: MFP.isMFPCategory({ categoryId: formData.categoryId })
					? 'mfp'
					: null
	}, [formData.categoryId])
