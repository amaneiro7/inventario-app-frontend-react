...
const ListWrapper = lazy(async () =>
	import('@/sections/components/ListComponent/ListWrapper').then(m => ({
		default: m.ListWrapper
	}))
)
const MainModelFilter = lazy(async () =>
	import('@/sections/components/ListComponent/MainModelFIlter').then(m => ({
		default: m.MainModelFilter
	}))
)
const ModelTable = lazy(async () =>
	import('./ModelTable').then(m => ({ default: m.ModelTable }))
)
const ModelDescription = lazy(async () =>
	import('./ModelDescription').then(m => ({ default: m.ModelDescription }))
)

export default function ListadoModelos() {
	const { inputData: initialInputData, defaultInputData } =
		useDefaultInitialInputValue()
	const { models, managePage, query, loading, addFilter, cleanFilters } =
		useModelContext()
