export class SiteAddress {
    ...
    public static isValid(value: string): boolean {
        ...
        const validLength = value.length >= SiteAddress.NAME_MIN_LENGTH && value.length <= SiteAddress.NAME_MAX_LENGTH
        if (!validLength) {
            errors.push(`${value} no es un nombre válido. Debe tener entre ${SiteAddress.NAME_MIN_LENGTH} y ${SiteAddress.NAME_MAX_LENGTH} caracteres`)
        }
