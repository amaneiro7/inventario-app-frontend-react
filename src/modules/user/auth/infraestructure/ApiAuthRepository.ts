...
export class ApiAuthRepository implements AuthRepository {
	async loginLocal({
		email,
		password
	}: Pick<UserPrimitives, 'email' | 'password'>): Promise<
		UserPrimitives & { accessToken: string }
	> {
	...
	async checkToken(): Promise<
		(UserPrimitives & { accessToken: string }) | null
	> {
	...
	async changePassword({
		...
		return await makeRequest({
			...
			method: 'PATCH',
			data: { password, newPassword, reTypePassword }
		})
	}
}
